...
from plancheck.grouping import build_clusters_v2, group_notes_columns, link_continued_columns
...
for h in headers:
    bb = h.bbox()
    texts = [b.text for r in h.rows for b in sorted(r.boxes, key=lambda b: b.x0) if b.text]
    print(f"  Header: {' '.join(texts)[:60]}  bbox=({bb[0]:.0f},{bb[1]:.0f},{bb[2]:.0f},{bb[3]:.0f})")

print(f"\nNotes blocks ({len(notes)}):")
for i, n in enumerate(notes):
    bb = n.bbox()
    texts = [b.text for r in n.rows for b in sorted(r.boxes, key=lambda b: b.x0) if b.text]
    print(f"  [{i}] rows={len(n.rows)} bbox=({bb[0]:.0f},{bb[1]:.0f},{bb[2]:.0f},{bb[3]:.0f}) "
          f"text={' '.join(texts)[:120]}")

cols = group_notes_columns(blocks, cfg=cfg)
print(f"\nNotes columns: {len(cols)}")
for col in cols:
    hdr = "orphan" if not col.header else " ".join(
        [b.text for r in col.header.rows for b in sorted(r.boxes, key=lambda b: b.x0) if b.text]
    )[:50]
    print(f'  Column "{hdr}": {len(col.notes_blocks)} notes')
    for j, nb in enumerate(col.notes_blocks):
        bb = nb.bbox()
        texts = [bx.text for r in nb.rows for bx in sorted(r.boxes, key=lambda bx: bx.x0) if bx.text]
        print(f"    [{j}] rows={len(nb.rows)} bbox=({bb[0]:.0f},{bb[1]:.0f},{bb[2]:.0f},{bb[3]:.0f}) "
...
for i, b in enumerate(blocks):
    ...
    if bb[0] > 2000 and bb[1] > 850:
        texts = [bx.text for r in b.rows for bx in sorted(r.boxes, key=lambda bx: bx.x0) if bx.text]
        print(f"  Block {i}: rows={len(b.rows)} hdr={b.is_header} note={b.is_notes} "
